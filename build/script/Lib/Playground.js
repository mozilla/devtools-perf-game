/*     
  ...
  ok + navigator.isCocoonJS
  ok + lock
  ok + release
  ok + state.proxy
  ok + mouse throttle
  + music / sound separation
  + fadeIn / fadeOut
  + soundFallback step
  + soundFallback cocoon
  + pointer
...
function Playground(args) {
  ...
  function step() {
    ...
    if (self.loader.count <= 0) {
      ...
      if (self.frameskip && self.tick % self.frameskip > 0) return;
      else if (self.frameskip) dt *= (self.frameskip);
...
playground.Mouse.prototype = {
  ...
  mousemove: playground.throttle(function(e) {
    ...
    this.parent.pointer.x = this.mousemoveEvent.x;
    this.parent.pointer.y = this.mousemoveEvent.y;
...
playground.Touch.prototype = {
  ...
  touchmove: function(e) {
    ...
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      var ttt = this.touches[touch.identifier];

      ttt.x = this.touchmoveEvent.x;
      ttt.y = this.touchmoveEvent.y;

      if (ttt.pointer) {
        this.parent.pointer.x = this.touchmoveEvent.x;
        this.parent.pointer.y = this.touchmoveEvent.y;
      }
  ...
  touchstart: function(e) {
    ...
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      if (e.touches.length === 1) this.touches[touch.identifier].pointer = true;

      this.pressed = true;

      this.trigger("touchmove", this.touchstartEvent);
...
Playground.Sound.prototype = {
  ...
  say: function(message) {
    ...
    if (!this.spu) this.spu = new SpeechSynthesisUtterance();

    this.spu.pitch = 1.5;
    this.spu.text = message;

    window.speechSynthesis.speak(this.spu);

    return;
    if (!this.speechPool[message]) {
      this.speechPool[message] = new SpeechSynthesisUtterance(message);
    }
    ...
    // msg.lang = "en-GB";
    // msg.rate = 1;
    window.speechSynthesis.speak(this.speechPool[message]);
  }
